...
Analysis of atmospheric feature occurrence and co-occurrence within IPCC AR6 Antarctic regions.
...
| `ANT_regions_cooccurrence_matrices.py` | Generates feature co-occurrence matrices for the top-100 EPEs per region, for ERA5 and RCM–ERA5 differences, with bootstrapping for significance testing. Requires CSV files from `ANT_regions_analysis_objects.py`. |
| `ANT_regions_occurrence_radar.py` | Generates radar plots of feature occurrence within the top-100 EPEs for each region, for ERA5 and RCMs, with bootstrapping. Requires CSV files from `ANT_regions_analysis_objects.py`. |
...
### 2. `EPE_Objects/`
Detection of extreme precipitation objects and association with atmospheric features.
...
| `EPE_feature_association_buffer_difference_part2.py` | Follows Part 1. Applies pixel-wise Wilcoxon signed-rank test to the computed differences and produces 6-panel plots. |
| `EPE_feature_association_buffer_composite.py` | Same as Part 2, but for all RCMs simultaneously, producing a single 6×3 composite figure (features × RCMs). Requires Part 1 to have been run for all target RCMs. |
| `EPE_frequency.py` | Computes multi-year frequency of EPE objects (both percentiles) and plots them. Applicable to all datasets individually. |
...
Computation and visualisation of multi-year feature climatologies.
...
Visualisation tools for case studies and animations.

| Script | Description |
|--------|-------------|
| `plot_case_studies.py` | Generates the case study plots used in the thesis, showing precipitation and feature overlays for selected Antarctic EPEs. Includes an anomaly comparison for the South Shetland Islands case. Modular and adaptable to additional cases or features. |
...
Computation of Integrated Vapour Transport (IVT) and its 95th percentile, used as input for AR detection.

| Script | Description |
|--------|-------------|
| `compute_ivte_ivtn.py` | Computes IVT east and north components from wind (ua, va) and specific humidity (hus) using the trapezoidal rule for vertical integration. Memory-efficient: loads one pressure level at a time. Applies a surface mask based on geopotential height. Originally written for RACMO2; logic is transferable to ERA5, HCLIM, and MetUM. Run after `prep_model_data.py`. |
| `ivt_computation.sh` | Computes IVT magnitude from the east/north components for each year using CDO. Checks for input file existence before running. **Step 1** of the IVT percentile pipeline. |
| `ivt_merge.sh` | Merges annual IVT magnitude files (2001–2020) into a single combined file per dataset. Sets the variable name to `ivt`. **Step 2** of the IVT percentile pipeline. |
| `ivt_percentile.sh` | Computes the 95th percentile of IVT over 2001–2020, using monthly min/max bounds. **Step 3 (final step)** of the IVT percentile pipeline. |

---

### 6. `Mean_Precipitation/`
Scripts for computing and comparing mean annual precipitation.
...
### 7. `MOAAP_code/`
Core feature detection framework, adapted for Antarctica.
...
Pipeline for preprocessing and embedding RCM data within the ERA5 background on the expanded PolarCORDEX domain.

| Script | Description |
|--------|-------------|
| `prep_model_data.py` | Processes geopotential height (zg) files from the specified RCM and years. Converts from metres to geopotential (m² s⁻²) if needed, to align with ERA5 convention. Backs up original files with a `.raw.nc` extension. **Step 1** of the embedding pipeline. |
| `prep_model_pr.py` | Processes RCM precipitation (pr) files. Converts units from kg m⁻² s⁻¹ to m (total hourly accumulation) and shifts time labels by +30 min to align with ERA5 convention. Backs up originals. **Step 1** of the precipitation embedding pipeline. |
| `remap_model_data.sh` | Remaps RCM output to the common PolarCORDEX grid using CDO. Iterates over years and variables, applies the chosen remapping method, and saves results in a structured directory. **Step 2** of the embedding pipeline. |
| `embed_model_data.py` | Embeds RCM data into the ERA5 background for the specified variables and years. Includes a blending mechanism (currently set to 0 points, i.e., no blending) and generates diagnostic plots to check for boundary discontinuities. **Final step** of the embedding pipeline. |
| `embed_model_pr.py` | Same as `embed_model_data.py`, but optimised for the `pr` variable (1-hourly with accumulations). Includes a 1-hour temporal trimming step for proper alignment. Generates diagnostic plots for boundary checks. |

---

### 9. `PR_Objects/`
Detection of 6-hourly precipitation objects and computation of feature contributions to mean precipitation.
...
| `PR_features_contribution.py` | Computes the contribution of atmospheric features to mean annual precipitation using a buffer-based approach on PR objects detected by `PR_Object_6hr_detection.py`. Produces 6-panel figures. |
...
Computation of seasonal precipitation percentiles used for EPE detection.
...
- **ERA5** reanalysis (Hersbach et al., 2020) — available via the [Copernicus Climate Data Store](https://cds.climate.copernicus.eu/)
- **PolarCORDEX RCMs**: HCLIM, MetUM, RACMO — available via the [PolarRES project](https://polarres.eu/)
- **IPCC AR6 Reference Regions** shapefiles — available at [IPCC-WG1/Atlas](https://github.com/IPCC-WG1/Atlas)

Processed intermediate outputs (NPZ files, NetCDF climatologies) are archived on Zenodo: [DOI: 10.5281/zenodo.XXXXXXX](https://doi.org/10.5281/zenodo.XXXXXXX)
...
> Prein, A. F., et al. (2023). MOAAP: Multi-Object Analysis of Atmospheric Phenomena. *[Journal]*, [volume], [pages]. DOI: [to be added]
